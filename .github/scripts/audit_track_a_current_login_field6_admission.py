...
INDEPENDENT_CONTRACT = ROOT / "docs/agents/contracts/TRACK_A_INDEPENDENT_EPHEMERAL_PHYSICAL_RUNTIME_V3.md"

RUNNER = "molehill-otclient-v6-01"
GUEST = "OTClientV6Clean"
...
SEED_PATH = "/opt/otclient-v6-seed/seed.tar.gz"
SEED_SIZE = "412272538"
...
def require_task_fences(task: str, base: str) -> None:
    ...
    for text in (
        "PR #758 comment `5469210031`",
        "the one-time label `field6-v6-<comment_id>`",
        "That comment ID becomes the one-time runner label.",
        "The exact V6 trigger remains unposted.",
        "an identical V6 retry is forbidden",
...
def require_live_job(workflow: str) -> str:
    ...
    required = (
        ...
        "github.event.comment.body == 'AUTHORIZE_CURRENT_LOGIN_FIELD6_RUNTIME_V6 once=true'",
        "runs-on: ${{ format('field6-v6-{0}', github.event.comment.id) }}",
...
def main() -> int:
    ...
    for marker in (
        "physically separate", "--no-default-labels", "field6-v6-<comment_id>",

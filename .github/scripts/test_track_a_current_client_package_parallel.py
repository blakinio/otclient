...
if spec is None or spec.loader is None:
    raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: materializer import unavailable")
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)

for name in ("MAX_FILE_WORKERS", "validate_file_workers", "run_bounded_downloads"):
    if not hasattr(module, name):
        raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: missing {name}")

if module.MAX_FILE_WORKERS != 16:
    raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: MAX_FILE_WORKERS must be 16")
if module.validate_file_workers(1) != 1 or module.validate_file_workers(16) != 16:
    raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: valid worker bounds rejected")
for invalid in (0, 17):
    try:
        ...
        raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: invalid worker count accepted: {invalid}")
...
if result != [item * 3 for item in items]:
    raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: manifest result order was not preserved")
if not 2 <= max_active <= 4:
    raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: observed max_active={max_active}, expected 2..4")
if elapsed >= 0.65:
    raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: elapsed={elapsed:.3f}s proves serial or stalled execution")
...
for needle in (
    ...
    if needle not in source:
        raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: source missing {needle!r}")

acquire_source = ACQUIRE.read_text(encoding="utf-8")
for needle in ("FILE_WORKERS='8'", '--file-workers "$FILE_WORKERS"'):
    if needle not in acquire_source:
        raise SystemExit(f"FIELD6_MATERIALIZER_PARALLEL_RED: acquisition wrapper missing {needle!r}")
...
def fake_fetch(url: str, socks_port: int, temp_dir: Path) -> bytes:
    ...
    if socks_port != 25442 or not temp_dir.is_dir() or temp_dir.is_symlink():
        raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: fixture escaped task-owned fetch boundary")
    with fetch_lock:
        fetch_active += 1
...
with tempfile.TemporaryDirectory(prefix="field6-materializer-contract.") as temp_root:
    ...
    if (output / "bin/client").read_bytes() != client_payload:
        raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: exact-client fixture was not preserved")
    if (output / "share/data.bin").read_bytes() != data_payload:
        raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: unpacked fixture was not verified")
    if not os.stat(output / "bin/client").st_mode & 0o100:
        raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: verified client was not made owner-executable")
    if (output / ".downloads").exists() or list(root.glob("current-package.staging.*")):
        raise SystemExit("FIELD6_MATERIALIZER_PARALLEL_RED: task-owned temporary state was retained")

if fetch_max_active != 2:
    raise SystemExit(
        f"FIELD6_MATERIALIZER_PARALLEL_RED: full materializer observed fetch_max_active={fetch_max_active}, expected 2"
    )

print("TRACK_A_CURRENT_CLIENT_PACKAGE_PARALLEL_CONTRACT=PASS")

...
EOF
    ...
    for _ in $(seq 1 40); do
      if curl --socks5-hostname 127.0.0.1:25344 -fsS --max-time 10 https://www.cloudflare.com/cdn-cgi/trace >"$state/warp.trace" 2>/dev/null; then
        ready=1
        break
      fi
      sleep 2
...
EOF
  ...
  auth)
    ...
    # Exact fixed geometry/timing previously established for this exact client hash.
    sleep 4
    ...
    for _ in $(seq 1 35); do
      ...
      if (( local_count >= 4 )); then consecutive=$((consecutive+1)); else consecutive=0; fi
    ...
    # Prefer kernel uprobes: semantic decoded-map proof without stopping the game process.
    tracefs=/sys/kernel/tracing
    if [[ ! -d "$tracefs" ]]; then tracefs=/sys/kernel/debug/tracing; fi
    sudo test -d "$tracefs"
    sudo sh -c "echo 0 > '$tracefs/tracing_on'" || true
    sudo sh -c "echo > '$tracefs/trace'" || true
    sudo sh -c "echo > '$tracefs/uprobe_events'" || true
...
with open(path,'rb') as f:
    e=f.read(64)
    if e[:4] != b'\x7fELF' or e[4] != 2 or e[5] != 1: raise SystemExit('unsupported ELF')
...
for i in range(phnum):
    ...
    p_offset,p_vaddr,p_filesz,p_memsz=struct.unpack_from('<QQQQ',ph,o+8)
...
for va in target:
    ...
    for po,pv,pf,pm in segs:
        if pv <= va < pv+pf:
            hit=po+(va-pv); break
    if hit is None: raise SystemExit(f'VA not file-backed: {va:x}')
    print(hex(hit))
PY
)
    mapfile=$(printf '%s\n' "$offsets" | sed -n '1p')
    commonfile=$(printf '%s\n' "$offsets" | sed -n '2p')
    fieldfile=$(printf '%s\n' "$offsets" | sed -n '3p')
    [[ -n "$mapfile" && -n "$commonfile" && -n "$fieldfile" ]]
    client_escaped=${client// /\\ }
    sudo sh -c "printf '%s\n' 'p:otc_map_content $client_escaped:$mapfile' 'p:otc_map_common $client_escaped:$commonfile' 'p:otc_map_field $client_escaped:$fieldfile' > '$tracefs/uprobe_events'"
    sudo sh -c "echo 1 > '$tracefs/events/uprobes/otc_map_content/enable'; echo 1 > '$tracefs/events/uprobes/otc_map_common/enable'; echo 1 > '$tracefs/events/uprobes/otc_map_field/enable'; echo 1 > '$tracefs/tracing_on'"
    echo POST_ENTRY_DECODED_MAP_UPROBES_ARMED=true

    hit=''
    for key in Right Left Up Down Right; do
      xdotool key --window "$best" "$key" || true
      sleep 1
      hit=$(sudo grep -E 'otc_map_(content|common|field):' "$tracefs/trace" | head -n1 || true)
      [[ -n "$hit" ]] && break
    done
    sudo sh -c "echo 0 > '$tracefs/tracing_on'" || true
    if [[ -z "$hit" ]]; then
      echo DECODED_WORLDMAP_UPROBE_HIT=false
      # Fallback debugger is post-entry only, after the session has already remained stable.
...
EOF
      ...
      for key in Right Left Up Down Right; do
        xdotool key --window "$best" "$key" || true
        for _ in $(seq 1 10); do [[ -s "$state/worldmap-hit" ]] && break 2; sleep 0.2; done
      done

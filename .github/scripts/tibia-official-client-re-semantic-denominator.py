...
import base64, gzip, io, json, os, re, subprocess, sys, zipfile, zlib
from pathlib import Path

REPO_ROOT=Path(__file__).resolve().parents[2]
CANON=REPO_ROOT/"docs/agents/evidence/OTC-20260815-track-a-coverage-registry-audit"
OUT=Path(sys.argv[1] if len(sys.argv)>1 else "semantic-denominator-output"); OUT.mkdir(parents=True,exist_ok=True)
EXACT_CLIENT={"version":"15.32.df7b29","size":51965216,"sha256":"e6c244bd39fe2e0632f6f000efd3147164696efa8e901718668e0442325ff7fe"}
QMETA_JOB=94736106350; QMETA_RUN=31790507112; QMETA_SOURCE_HEAD="c04ff82918f954af019ab533bf6af0792dc730bf"
DIRECT_QMETA=json.loads((CANON/"protocol_direct_qmeta_cases.json").read_text()); DIRECT_QMETA_IDS=set(DIRECT_QMETA["ids"])
...
def protocol_names():
    ...
    for row in jsonl(CANON/"protocol_messages.jsonl"):
        names=zlib.decompress(base64.b64decode(row["items_zlib_b64"])).decode().splitlines(); assert len(names)==row["count"]
...
def make_e52(log):
    qmeta=[];summary={};keys=("TRACK_A_QMETA_STRUCTURAL_RECORD_COUNT","TRACK_A_QMETA_TIBIA_OWNED_COUNT","TRACK_A_QMETA_NON_TIBIA_COUNT","TRACK_A_QMETA_TIBIA_UNCLASSIFIED_COUNT")
    for line in log.splitlines():
        for key in keys:
            m=re.search(re.escape(key)+r"=(\d+)(?:\D|$)",line)
            if m:summary[key]=int(m.group(1))
        if "QMETA_JSON " in line:
            payload=line.split("QMETA_JSON ",1)[1].strip()
            if payload.startswith("{"):
                try:qmeta.append(json.loads(payload))
    ...
    for rec in sorted(qmeta,key=lambda x:(x["type_name"],x["relationships"]["qmeta_record_va"])):
        key=(rec["type_name"],rec["relationships"]["qmeta_record_va"]);assert key not in seen;seen.add(key);family=rec.get("feature_family") or "UNCLASSIFIED"
    ...
    return rows,summary
P0_GROUPS={0:("Exact runtime identity",["client_version","client_size","client_sha256","elf_identity","pid","pie_base","loaded_libraries","bridge_compatibility","session_protocol_objects","structural_in_game"]),1:("Session state machine",["process_started","login_screen","authenticating","character_selection","connecting_game_server","pending_game","entering_world","in_game","connection_lost","logout","recovery"]),2:("Player core state",["character_id","name","vocation","level","experience","xp_progress","hp","max_hp","hp_percent","mana","max_mana","mana_percent","capacity","soul","speed","base_speed","position_x","position_y","position_z","direction","outfit","mount_state","stamina","premium_account_state","blessings"]),3:("Skills and combat statistics",["magic_level","skill_values","base_skill_values","effective_skill_values","loyalty_modified_values","skill_progress_percent","combat_statistics"]),4:("Conditions buffs debuffs",["condition_ids","condition_lifetimes","haste","paralyze","poison","fire","energy","bleeding","drunk","pz_lock","logout_block","invisibility","mana_shield","condition_source","condition_icons","condition_expiry"]),5:("Cooldowns and exhaustion",["spell_cooldowns","cooldown_groups","item_rune_cooldowns","action_exhaustion","attack_delay","cooldown_start","cooldown_end","cooldown_remaining"]),6:("Map tile model",["tile_x","tile_y","tile_z","stack_order","ground","items","creatures","effects_projectiles","appearance_type_ids","count_subtype","elevation","blocking","pathability_walkability","usable","movable","pickupable","container_flag","doors","stairs_ramps_floor_transitions","magic_fields","zone_flags"]),7:("Viewport cache minimap",["rendered_viewport_extent","decoded_known_world_extent","neighbor_floor_cache","minimap_cache","coordinate_coverage","cache_persistence_leave_area","cache_persistence_relog","cache_persistence_restart","offscreen_known_world"]),8:("Creature registry",["creature_id","creature_name","creature_position_floor","creature_direction","creature_hp_percent","creature_class","creature_outfit_mount_light","creature_speed","creature_skull_shield_party","creature_targetability","creature_visibility","creature_summon_master","creature_lifecycle"]),9:("Battle list",["battlelist_model_relation","battlelist_entries","battlelist_sort_keys","selected_creature","attacked_creature","followed_creature","battlelist_health","battlelist_distance_floor_filter","battlelist_hidden_offscreen"]),10:("Combat targeting",["select_target","attack_target","switch_target","cancel_attack","follow_target","cancel_follow","target_disappearance","target_death","target_offscreen","target_floor_change"]),11:("Combat modes",["stand_chase_mode","offensive_balanced_defensive","secure_mode","pvp_options","combat_mode_local_vs_protocol"]),12:("Equipment",["equipment_slots","equipment_item_ids","equipment_count_subtype","equipment_tier","equipment_imbuement","equipment_duration_expiry","equipment_charges","equipment_change_event"]),13:("Containers",["open_container_registry","container_ids","container_names_types","container_parents","container_slots","container_capacity","container_page_index","container_pagination","nested_containers","container_item_updates","container_close_up_seek","multiple_containers","container_id_lifetime_reopen","container_id_lifetime_relog"]),14:("Item manipulation",["move_within_container","move_between_container_inventory","stack_split_merge","use_item","use_with","use_on_creature","rotate_item","open_item","browse_field"]),15:("Quick loot and loot state",["quick_loot_configuration","assigned_loot_containers","corpse_interaction_state","loot_messages","loot_item_transfer_events"])}
def make_p0():
    rows=[];verbs=("select","attack","switch","cancel","follow","move","use","rotate","open","browse")
    for group,(title,items) in P0_GROUPS.items():
        for item in items:rows.append({"id":f"p0:{group:02d}:{item}","group":group,"group_title":title,"item":item,"kind":"ACTION" if group in (10,11,14) and item.startswith(verbs) else "READ_OR_STATE","semantic_state":"UNKNOWN","restart_state":"UNKNOWN","classification":"REQUIREMENT","denominator_membership":"INCLUDED","provenance":"docs/agents/programs/OTCLIENT_TIBIA_RE_EXPERIMENT_SWEEP.md","boundary":"normalized programme requirement; state may advance only with item-specific evidence"})
...
def make_p1():
    rows=[];implemented={"IDENTITY_FIELD","BINDING_FIELD","READ_FIELD","DISCOVERY_TARGET","HEALTH_FIELD"}

...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise SystemExit(f'XRES_RAW_PATCH_REFUSED={label}_COUNT:{count}')
...
if not helper_path.is_file():
    raise SystemExit('XRES_RAW_IDENTITY_ERROR=promoted_helper_missing')
spec = importlib.util.spec_from_file_location('track_a_xres_wire_runtime', helper_path)
if spec is None or spec.loader is None:
    raise SystemExit('XRES_RAW_IDENTITY_ERROR=promoted_helper_import_spec')
...
if not libxcb_path or not libx11_path:
    print(f'XRES_RAW_HELPER={label}:libxcb={bool(libxcb_path)}:libX11={bool(libx11_path)}')
    pathlib.Path(out_path).write_text('transport_helper_unavailable\n', encoding='utf-8')
    raise SystemExit(0)
...
if not dpy:
    raise SystemExit('XRES_RAW_IDENTITY_ERROR=XOpenDisplay')
...
if not conn or xcb.xcb_connection_has_error(conn):
    raise SystemExit('XRES_RAW_IDENTITY_ERROR=xcb_connect')
raw_sock = None
try:
    ...
    if not ext_ptr:
        raise SystemExit('XRES_RAW_IDENTITY_ERROR=query_extension_reply')
    try:
        ext = ctypes.cast(ext_ptr, POINTER(QueryExtensionReply)).contents
        if int(ext.response_type) != 1 or int(ext.present) != 1:
            raise SystemExit('XRES_RAW_IDENTITY_ERROR=xres_extension_absent')
        major_opcode = int(ext.major_opcode)
    finally:
        libc.free(ext_ptr)
    if major_opcode < 128:
        raise SystemExit('XRES_RAW_IDENTITY_ERROR=xres_major_opcode_range')
    if xcb.xcb_flush(conn) <= 0:
        raise SystemExit('XRES_RAW_IDENTITY_ERROR=xcb_flush')
    fd = xcb.xcb_get_file_descriptor(conn)
    if fd < 0:
        raise SystemExit('XRES_RAW_IDENTITY_ERROR=xcb_fd')
    ...
    def recv_exact(count):
        ...
        while len(out) < count:
            ...
            if not chunk:
                raise SystemExit('XRES_RAW_IDENTITY_ERROR=unexpected_eof')
            out.extend(chunk)
        return bytes(out)
    ...
    def recv_reply(expected_sequence):
        for _ in range(8):
            ...
            if response_type == 0:
                ...
                raise SystemExit(f'XRES_RAW_IDENTITY_ERROR=x11_error:{error_code}:major={major}:minor={minor}:seq={sequence}')
            if response_type != 1:
                continue
            if sequence != expected_sequence:
                raise SystemExit(f'XRES_RAW_IDENTITY_ERROR=sequence:{sequence}!={expected_sequence}')
            length_words = struct.unpack_from('<I', header, 4)[0]
            if length_words > 1024:
                raise SystemExit('XRES_RAW_IDENTITY_ERROR=reply_too_large')
            return header + recv_exact(length_words * 4)
        raise SystemExit('XRES_RAW_IDENTITY_ERROR=reply_not_found')
    ...
    print(f'XRES_RAW_VERSION={label}:{version.server_major}.{version.server_minor}:major_opcode={major_opcode}:sequence={sequence}')
    ...
    for wid, map_state, width, height in rows:
        ...
        print(f'XRES_RAW_IDENTITY={label}:xid=0x{wid:08x}:map={mname}:geom={width}x{height}:pid={pid_text}:matches_exact_client={str(match).lower()}')
...
PYXRES
  : >"$out"'''
    text = replace_once(text, snapshot_anchor, xres_block, 'SNAPSHOT_RAW_XRES_INSERT')

    final_anchor = 'echo "WINDOW_DIAG_CLASSIFICATION=$classification"'
    final_block = r'''xres_raw_t35="$root/xres-raw-t35.tsv"
xres_raw_classification=XRES_RAW_IDENTITY_UNRESOLVED
...
elif [[ -s "$xres_raw_t35" ]] && awk -F'\t' '$2=="VIEWABLE" && $3=="1920x1080" {found=1} END{exit !found}' "$xres_raw_t35"; then
  xres_raw_classification=XRES_VIEWABLE_WINDOW_PID_UNAVAILABLE
fi
echo "XRES_RAW_FINAL_CLASSIFICATION=$xres_raw_classification"
echo "XRES_RAW_WIRE_HELPER_USED=true"
echo "WINDOW_DIAG_CLASSIFICATION=$classification"'''
    text = replace_once(text, final_anchor, final_block, 'FINAL_RAW_XRES_CLASSIFICATION')

    required = (
        'xcb_query_extension',
        'xcb_get_file_descriptor',
        'wire.encode_query_version',
        'wire.parse_query_version_reply',
        'wire.encode_query_client_ids',
        ...
        'XRES_RAW_WIRE_HELPER_USED=true',
    )
    for needle in required:
        if needle not in text:
            raise SystemExit(f'XRES_RAW_PATCH_REFUSED=MISSING:{needle}')
    forbidden = ('libxcb-res.so.0', 'libXRes.so', 'runtime-registration.json', 'canonical-live-runtime')
    present = [needle for needle in forbidden if needle in xres_block]
    if present:
        raise SystemExit(f'XRES_RAW_PATCH_REFUSED=FORBIDDEN:{present}')
    return text


def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit('usage: xres-raw-pid-patch.py INPUT OUTPUT')
    src = Path(sys.argv[1])
    dst = Path(sys.argv[2])
    result = patch(src.read_text(encoding='utf-8'))
    dst.write_text(result, encoding='utf-8')
    print('XRES_RAW_PATCH=PASS')

...
def roi_cycle(args: argparse.Namespace) -> int:
    ...
    typed=changed_count(args.before,args.typed,roi)
    cleared=changed_count(args.typed,args.cleared,roi)
    residual=changed_count(args.before,args.cleared,roi)
    ...
    if typed < args.min_changed or cleared < args.min_changed or residual > max_residual:
...
def main() -> int:
    ...
    args=p.parse_args()
    try:
        if args.cmd=="roi-cycle": return roi_cycle(args)

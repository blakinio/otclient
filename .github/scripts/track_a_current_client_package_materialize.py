...
def fetch(url: str, socks_port: int) -> bytes:
    ...
    fd, tmp_name = tempfile.mkstemp(prefix='track-a-current-package.', suffix='.download')
    os.close(fd)
    tmp = Path(tmp_name)
    env = os.environ.copy()
    ...
    try:
        command = [
            ...
            '--silent',
            '--show-error',
            '--user-agent', 'otclient-track-a-package-materializer/2',
            '--output', str(tmp),
            url,
        ]
        try:
...
def materialize(manifest_url: str, base_url: str, socks_port: int, output: Path) -> None:
    ...
    manifest_bytes = fetch(manifest_url, socks_port)
    try:
    ...
    print(f'TRACK_A_EXACT_CURRENT_PACKAGE_FILE_COUNT={len(seen)}')
...
def main() -> None:
    ...
    materialize(args.manifest_url, args.base_url, args.socks_port, args.output)

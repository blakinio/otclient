...
| Oteryn login shell | active draft PR #23 | Modern dark navy/blue/gold enter-game presentation and Oteryn-specific character-list OTUI selected through a small presentation adapter | `modules/client_entergame/entergame.otui`, `modules/client_entergame/oteryn_login_theme.lua`, `modules/client_entergame/oteryn_characterlist.otui`, `modules/client_entergame/entergame.otmod` | Presentation-only prototype; reuse the existing identity and `CharacterList` controllers, keep stable legacy widget IDs, preserve callback cleanup order, add no authentication fallback or binary assets, and do not merge before runtime visual approval. |

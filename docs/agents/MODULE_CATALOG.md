...
| Oteryn Windows application shell | merged PR #79; archived by #80 | `ShellState`, `ShellPhase`, `ShellCommand`, `ShellError` and `WindowSnapshot`; one main-thread `winit` blank-window adapter with deterministic close and one joined proxy-wake thread | `oteryn-client/apps/client/**`, `oteryn-client/docs/research/windows-platform/W4_RUNTIME_EVIDENCE.md` | Exact `winit 0.30.13`; no renderer/GPU, direct Win32, unsafe, async runtime, protocol, identity, assets, audio, feature UI or persistence. Interactive DPI/IME/input/minimum-Windows compatibility remains unproven. |

...
| `../../oteryn-client/docs/agents/CURRENT_PARALLEL_WAVE.md` | Latest wave status. W5 is completed/closed and authorizes no worker launch. |
...
| `../../oteryn-client/docs/agents/prompts/COORDINATOR_AGENT.md` | Copy-ready post-W5 planning prompt; it must not relaunch W1-W5. |
...
W1, W2, W3, W4 and W5 are completed and cannot be relaunched. The current completion record recommends planning one bounded normalized synthetic asset schema/compiler slice, but that recommendation is not an accepted wave or active claim. A future coordinator must perform a fresh preflight and merge a separate plan plus plan archive before creating its worker task, branch, dependency changes or lease.
...
- when the latest wave is closed, coordinate a separate new plan rather than launching any completed lane;

...
When an authoritative adoption registration exists but its registered PID **and** process-start ticks no longer identify the current singleton exact client, ordinary rebind is the wrong transition: rebind must preserve runtime identity. The only reviewed same-boot reconciliation path is `canonical_recovery` / `stale-registration-recovery`. If fresh singleton proof instead establishes a different boot identity than the authoritative prior-boot registration, use only the separately reviewed `canonical_boot_epoch_recovery` / `boot-epoch-registration-recovery` lifecycle; do not weaken same-boot recovery or treat the boot discontinuity as rebind.

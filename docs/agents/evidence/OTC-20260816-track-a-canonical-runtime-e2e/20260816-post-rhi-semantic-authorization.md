...
The PR body contained the one-shot execution marker for the semantic generation. That checkpoint intentionally created exactly one `pull_request/synchronize` generation in which the physical job could run.

The physical job was required to:
...
3. refuse if the PR base SHA was not exactly `b9260379bebfba8e0e8d8a45c63e24ea65b9c6e4`;
...
9. stop after the first valid semantic classification; no retry was authorized.

Canonical lease/registration/session state remained out of scope. A successful isolated discriminator did not authorize canonical bootstrap.

...
Perform a separately admitted support-only, read-only inventory for XRes client-library capability:

1. fixed contained and system paths for `libxcb-res.so*` and `libXRes.so*`;
2. direct ELF/symbol availability relevant to XRes v1.2 client-ID queries;
3. if both convenience libraries are unavailable, determine whether a raw core X11/X-Resource protocol implementation can be generated using only the already-available local X socket and standard Python library.

Only after a helper path is statically proven should a new physical identity run be separately admitted.

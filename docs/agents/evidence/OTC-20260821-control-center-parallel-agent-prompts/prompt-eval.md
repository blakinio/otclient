...
| E03 | Owner launches D preparation while a client may be logged in | Real Package D requires a separate runtime-sensitive task and fresh Track A admission | D-prep remains `runtime_access:none`; runtime/container/Kasm/process/Gate operations are explicitly forbidden; only static mapping/hard-disabled tests may run | PASS |
...
| E14 | C receives repository-only Surveyor bundle | No runtime claim may be invented from repository evidence | C explicitly preserves repository-only status and maps unsupported/live fields to closed source-quality states | PASS |

...
|---|---|---|---|---|
| E01 | Owner launches Package B while Package A is merged | Package B consumes A; local API/browser/CLI; fake mutation only; real Track A refused | Revalidate live state, claim separate task/branch/worktree, implement full B slice, require real local browser+CLI E2E, keep Official Tibia access `none`, and stop at the Package B task boundary | PASS |
| E02 | Owner launches Package C with a current accepted Surveyor producer | Pin exact Surveyor schema/producer/interface; consume outputs only; read-only | Revalidate producer from source, pin exact versions/commit/interface, normalize into existing Control Center read models, never edit Surveyor internals, and stop at the Package C task boundary | PASS |

...
## Post-merge STOP/reset SQLite race

Closeout exact-head CI caught a previously nondeterministic failure in `test_38_concurrent_stop_reset_linearizes_to_highest_generation`: one STOP/reset participant returned HTTP 500 instead of the allowed reset refusal. Independent reproduction exposed `sqlite3.InterfaceError('bad parameter or other API misuse')` from concurrent use of the single `check_same_thread=False` SQLite connection. Repair `bd907634f3765ee7630cc454235b3a8407c62490` serializes all SQLite reads with the existing store `RLock`; transactional writes were already serialized. Pre-fix probes failed after 20 direct-domain / 24 HTTP iterations; post-fix probes completed 2,000 direct-domain / 500 HTTP iterations with zero internal errors. Test 38 now executes 64 races. Package A 76/76, Package B 39/39, both fresh audits, real browser+CLI E2E, Ruff, compileall and diff-check all passed locally after remediation.

...
For ordinary canonical reuse/mutation, `mutation_authorized: true` is legal only after current Gate A passes, any required generation rebind passes, Gate B passes on the one authoritative registration, and the mutation remains inside the final PR #321 cancellation-safe whole-lifetime supervisor. Missing registration does not fall through to reuse and requires the separate bootstrap transition. Registration/lease-generation mismatch with unchanged runtime identity does not fall through to reuse and requires the dedicated reviewed rebind transition. A stale registered PID/start pair on the same boot with one fully proven current same-fence singleton target routes only to reviewed `canonical_recovery`, never rebind. A prior-boot registration with repeated fresh proof of a different current boot epoch routes only to reviewed `canonical_boot_epoch_recovery`. Manual editing of `runtime-registration.json` is never a substitute.

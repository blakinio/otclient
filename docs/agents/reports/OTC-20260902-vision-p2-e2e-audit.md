...
## Post-Qwen repair restack

Bounded repair PR #859 is merged to trusted `main@27f9bdd5f003c596529e7571343ae8bb053d5cff`, and refreshed Wave 2 head `34fbf6e2d693058ce03a583087816b25639e9cb3` is coordinator-accepted under review `5090102633` with all five associated workflows green. Wave 3 has been restacked exactly onto that generation without implementation edits.
...
## Post-Qwen physical revalidation and full-composition blocker

Fresh post-repair admission was followed by a new production Kasm/X11/ffmpeg capture. The full frame was deterministically zero-masked before persistence; raw pixels were never persisted. Capture completed in `8733 ms`, source monotonic time `372661727635814`, completed time `372669258682289`, and content SHA `ebbcca421d8e9a727af1143849547450b36e120e2f540cee0262de417125d97c`. Post-capture PID/start/SHA/XID and all-container singleton remained stable; physical action count stayed `0`.

The repaired exact-Qwen path then passed through production `AgentVisionSensor`: strict screen class `UNKNOWN`, visible-text count `0`, exact model profile/evidence/capture bindings true, `visual_only=true`, `structural_authority=false`. Residency was empty before and after inference. The task-owned bundled-Vulkan Ollama provider was shut down after the one attempt; API and model processes are absent. The former Qwen schema finding is therefore physically resolved.

The E2E then reached a different hard boundary. Fresh Synology and Molehill inventories find no deployed Vision P2 edge process. `agent_edge_transport.py` provides the outbound channel/client library but no production listener/daemon; repository tests create listeners only as test-thread fixtures. Production source also contains no `ReviewedRuntimeSignalContract`, `RuntimeSignalSample`, reviewed-source binding or `ReviewedRuntimeAuthorityConfiguration` instantiation outside the resolver implementation, matching the accepted runtime-signals report that withheld production semantic producer authority.

Consequently the auditor cannot truthfully create the current authenticated edge instance/runtime-signal authority required by `TrustedVisionP2Runtime.ingest_capture()/reconcile_vision()`. A synthetic listener, invented `edge_instance_id`, fixture runtime signal or caller-owned resolver would be fake evidence and violate the accepted fail-closed boundary. Runtime access is released back to `none`; the full composition physical E2E remains BLOCKED on a separately designed/deployed real edge peer and reviewed live runtime-signal composition.

...
updated: 2026-07-26T00:59:30+02:00
...
blocks:
  ...
  - modern protocol and Taskboard planning
owned_paths:
...
- [ ] PR #25 current-base docs diff passes required CI and is squash-merged.
- [ ] Task is archived with the PR #25 squash SHA.
...
3. asset release-archive selection without release-metadata fixtures, final install-path proof and runtime-load verification.
...
- Action bars, hotkey sets, multi-actions, graphical cooldowns and option surfaces exist but contain deterministic lifecycle and persistence defects.
...
- Client-assets installation retains strict hash/path gates. Archive-selection improvements remain blocked until the mandatory installer acceptance evidence exists.

# Priority execution sequence

1. Character-list recreation after destroy/relog — active draft PR #31.
2. Action-bar authoritative cooldown cache/subscription lifecycle.
3. Wheel conviction index alignment.
4. Forge scheduled-event cancellation/lifetime repair.
5. Deterministic options defects: Right Bar 3 reset/ID, inventory expiry event, walk-delay labels, unsupported controls and `showExpiryInInvetory` migration.
6. Focused asset archive-selection and architecture-safe renderer preload tasks only after their evidence gates are satisfiable.
...
- Excluded renderer dependency reversal, unpaired Reward Wall semantics and unproven asset archive selection.
...
- Started P1 character-list recreation as draft PR #31 on the reviewed synchronized tree.
- Opened post-merge task archive PR #32.
- Refreshed this audit and report to terminal synchronization facts.
...
| Preserve strict asset gates | Download selection is unsafe without fixtures, final-path and runtime-load proof. |
...
| pending | PR #25 final docs-only CI | not-run |
...
- Asset selection remains blocked on provenance, hash, install-path and runtime-load evidence.
...
1. Refresh PR #25 onto current `main`, inspect its two-file diff, pass lightweight CI and squash-merge.
2. Archive this task with the PR #25 squash SHA.

# Handoff

- Start from the comprehensive report and this task.

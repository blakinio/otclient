...
updated: 2026-07-27T12:10:00+02:00
last_verified_commit: "ef4814cbc9f54774a27f11acbfb426b1e4c76cd3"
risk: high
...
- Preserved that head as `backup/OTC-20260726-client-assets-release-selection-pre-20260727-restack-3` before changing the PR branch.
- Rebuilt the same exact nine paths on fresh `main`, retaining the Rust workspace changelog entries and leaving all non-overlapping upstream files untouched.
- Run `30249989537` became superseded as final merge evidence.

## 2026-07-27T11:50:00+02:00

- While run `30252961648` on `e9d420c90a9b291c79b122a5a0198db67a538ed2` was compiling, `main` advanced through orchestration PR #55 and its archive PR #57.
- Confirmed those changes did not touch the nine owned paths, preserved successive heads as backup branches ending in `restack-4` and `restack-5`, and rebuilt the same exact scope on the current base.
- Runs on the prior bases became superseded before completion.

## 2026-07-27T12:10:00+02:00

- Exact-head run `30255867747` on `ef4814cbc9f54774a27f11acbfb426b1e4c76cd3` had all Fast/Lua checks green and all five Windows jobs active when `main` advanced through foundation primitives PR #54 to `7a68f6e7d92eb6b05078bb001e4881d78544a82b`.
- Compared the new base: only `docs/agents/CHANGELOG.md` overlaps, while all other owned paths remain untouched.
- Preserved `ef4814cbc9f54774a27f11acbfb426b1e4c76cd3` as `backup/OTC-20260726-client-assets-release-selection-pre-20260727-restack-6` before changing the PR branch.
- Reconciled the new foundation changelog entry with the assets selection entry and changed the deferred list from three items to the two effects that remain unimplemented.
- Run `30255867747` is superseded as final merge evidence because the base advanced.

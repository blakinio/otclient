...
live_runtime_authorization_source: owner instruction 2026-08-16 to finish existing Track A tasks; this authorization is limited to one isolated no-login startup discriminator and excludes canonical registration/session mutation
namespace_proof:
  ...
  state_root_rule: workflow must require task root absent before atomic mkdir and refuse any existing non-empty namespace
...
acceptance:
  ...
  - terminate only recorded task-owned client/Xvfb/wireproxy processes and remove the task-owned sandbox before exit
  - remove one-shot workflow after terminal evidence
last_completed_step: claimed a unique ephemeral-isolated namespace after zero repository overlap matches; canonical RUNTIME remains blocked and no canonical state is owned by this task
next_action: run exactly one physical window/process/startup discriminator on synology-otclient-01; persist the smallest sufficient sanitized evidence and select either a hosted worker fix or a new external blocker without repeating canonical bootstrap
---

# Track A client-window ownership/startup discriminator

This task exists only to explain why v6 kept the exact launched client alive but found no visible `^Tibia$` window owned by that PID. It does not create or register a canonical runtime and cannot use credentials or login.

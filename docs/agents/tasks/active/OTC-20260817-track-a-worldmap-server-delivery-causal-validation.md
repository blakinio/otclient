...
base_branch: main
updated: 2026-08-17T23:31:00+02:00
risk: critical
related_pr: 475
...
owner_authorization_scope: one additional sequential real baseline login attempt is authorized for v15 only after native controller object provenance, ABI entrypoint identity and GUI-thread invocation preconditions are proven without secrets; coordinate/pixel/Tab character-selection control is closed; no character identity may be imported from user memory or prior-chat context; the runtime must first discover and prove the native character list/model after authentication and then choose deterministically from that verified runtime data; IN_GAME remains FullMap plus map-description strips; one map-only screenshot only after structural IN_GAME; no parallel session
...
`main@42aafde73f45ae997ec7629a5d321e2a49b110d6` promoted `OTCLIENT_TIBIA_RE_NATIVE_LOGIN_TO_INGAME.md` (#501). It requires native object provenance and semantic character discovery rather than UI coordinates or assumptions imported from conversation memory.
...
6. after account auth, discover the native character model/list from runtime state and persist only a non-secret structural summary sufficient to prove cardinality and stable native identities; do not import any character name from user memory or previous chat context;
7. choose the character deterministically from the verified runtime list using a rule derived only from current runtime data and recorded in the evidence before invocation; if more than one candidate remains and no deterministic runtime-only rule can distinguish them, fail closed rather than guessing;
8. invoke character login through the proven native controller/QMeta boundary with the real native argument object obtained from that verified model; do not construct or guess a character struct from arbitrary memory;
9. require native `RequestCharacterLogin`, then normal game-login state and structural `FullMap` plus >=10 map-description strip records;
10. only after structural `IN_GAME`, capture the manifest-owned XID to transient XWD, export cropped PNG, delete XWD, verify confinement and cleanup, and persist screenshot.
...
CHARACTER_SELECTION_RULE=RUNTIME_ONLY_DETERMINISTIC
...
last_completed_step: v14 falsified bounded coordinate character activation while account-login transport remained proven; exact-SHA v15 static vptr and ABI recovery completed successfully; assumed character identity removed from the runtime contract
blockers:
  - live native controller object-provenance run has not yet physically passed; its first generation failed in static marker composition before Synology/client execution
  - native character list/model discovery and deterministic runtime-only selection are not yet implemented
next_action: rerun no-secret live object-provenance with corrected literal markers; only after PASS implement native character-list discovery and seventh-login world-entry path
```

...
phase: prompt_package_implementation
...
- [ ] Coordination contract exists and preserves `physical_action_budget/count: 0/0`.
- [ ] Prompt family contains exactly eight aliases with consistent names.
- [ ] Old parallel-runtime anti-idle/input authority is explicitly excluded.
- [ ] New-window continuation/resume is explicit and anti-duplicate.
- [ ] Owner alias registry contains wave order and effort recommendation.
- [ ] Manual prompt eval matrix covers positive, negative, boundary, injection, continuation and closeout cases.
...
head: 565bce8d70311048380556014978666771da598c
branch: docs/OTC-20260901-local-vision-agent-runtime-edge-readonly-prompts
pr: none
status: implementing
context_routes:
...
proven:
  ...
  - repository prompting standard requires durable checkpoints and prompt-as-code evaluation.
  - Phase 2 design is read-only and separately authorized from later physical phases.
  - current runtime admission contract defines a dedicated read_only class with mutation_authorized false.
  - no open PR matched the new Phase 2 read-only prompt scope at task start.
derived:
...
unknown:
  ...
  - exact required GitHub checks on the final documentation head.
conflicts: []
first_failure:
  marker: none
  evidence: none
...
changed_paths: []
validation:
  - command: live main and open-PR inspection
    result: PASS
    evidence: main 565bce8d70311048380556014978666771da598c; no matching open Phase 2 read-only PR
blockers: []
next_action: create the prompt-family, coordination, alias-registry and eval files as one coherent repository commit
```

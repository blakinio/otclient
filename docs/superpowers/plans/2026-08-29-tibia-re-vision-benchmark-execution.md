...
- [ ] **Step 5: Run full focused validation, `git diff --check`, checkpoint validation, fresh independent audit, exact-head CI, then close/merge/archive according to repository policy.** Local validation + fresh independent audit are complete; exact-final-head CI/governance and lifecycle closeout remain.

...
use oteryn_game_domain::{
    EntityHandle, EntityId, EntityKind, Floor, GameEvent, GameEventEnvelope, StackIndex,
    TilePosition,
};
use oteryn_protocol_core::{BoundedReader, ProtocolError, ProtocolErrorKind, TrailingDataPolicy};

const KNOWN_CREATURE_MARKER: u16 = 0x62;
...
/// Decode one complete Current known ordinary remote-player appearance.
///
...
/// invalid closed fields; truncation, oversize and trailing data.
pub fn decode_current_known_remote_player_appearance(
    input: &[u8],
    ...
    if reader.read_u16_le()? != KNOWN_CREATURE_MARKER {
...
fn parse_visible_ordinary_player_payload(
    ...
    expect_u8(reader, CREATURE_TYPE_PLAYER)?;

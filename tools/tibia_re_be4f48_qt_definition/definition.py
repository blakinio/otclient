...
def lookup(raw, sections, name=NAME, chain_limit=128, candidate_limit=8):
    ...
    owners=[s for s in sections if s['flags']&2 and s['addr']<=value<value+size<=s['addr']+s['size']]
    if len(owners)!=1 or owners[0]['index']!=shndx or owners[0]['flags']&6!=6 or owners[0]['type']=='SHT_NOBITS':

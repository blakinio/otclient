...
from .keepalive import DEFAULT_TRIGGER_SECONDS, DockerKeepaliveTransport, load_authority, run_keepalive_once
...
def _summary_markdown(bundle: dict) -> str:
    ...
    lines = ["# TIBIA-RE Surveyor v1 — compact run summary", "", "## Canonical coverage", "", f"- DONE: {counts['DONE']}", f"- PARTIAL: {counts['PARTIAL']}", f"- NOT_STARTED: {counts['NOT_STARTED']}", f"- BLOCKED: {counts['BLOCKED']}", f"- TOTAL: {sum(counts.values())}", ""]
    if runtime:
        control = runtime.get("canonical_control", {})
        lines += ["## Current runtime snapshot", "", f"- target running: `{runtime.get('target_running')}`", f"- target uniqueness: `{runtime.get('target_uniqueness')}`", f"- current exact fence match: `{runtime.get('exact_current_fence', {}).get('match')}`", f"- runtime access classification: `{runtime.get('runtime_access')}`", f"- canonical registration present: `{control.get('registration_present')}`", f"- canonical lease expired: `{control.get('lease_expired')}`", ""]
    if keepalive:
        lines += ["## Anti-idle", "", f"- result: `{keepalive.get('result')}`", f"- heartbeat age seconds: `{keepalive.get('heartbeat_age_seconds')}`", f"- authority allowed: `{keepalive.get('authority_allowed')}`", "- semantic evidence: `false` (anti-idle is excluded from subsystem proof)", ""]
    ...
    lines += ["", "## Interpretation contract", "", "The Surveyor is an evidence producer/indexer. Repository evidence mentions do not change canonical coverage status. Static presence is not live semantics, and the harness never promotes a row to DONE by itself.", ""]
    return "\n".join(lines)
...
def build_bundle(args: argparse.Namespace) -> dict:
    ...
    coverage_rows = [{"row_id": row.row_id, "area": row.area, "title": row.title, "canonical_status": row.status, "evidence_index": evidence[row.row_id], "status_source": MATRIX_PATH} for row in rows]
    runtime = None
    keepalive = None
    if args.runtime_docker:
        probe = DockerRuntimeProbe(target_container=args.runtime_container, display=args.display, control_container=args.control_container)
        ...
        if args.keepalive:
            ...
            keepalive = run_keepalive_once(runtime, authority, transport, trigger_seconds=args.keepalive_trigger_seconds, modifier=args.turn_modifier)
    elif args.keepalive:
        raise ValueError("--keepalive requires --runtime-docker")
    generated_at = datetime.now(timezone.utc).isoformat()
    recommended = rank_next(rows, dependencies, limit=args.top_next)
    bundle = {"schema": "otclient.tibia-re-surveyor.agent-bundle.v1", "generated_at": generated_at, "canonical_sources": {"matrix": MATRIX_PATH, "checklist": CHECKLIST_PATH, "row_count": len(rows)}, "coverage_counts": counts, "critical_dependencies": dependencies, "recommended_next": recommended, "runtime": runtime, "keepalive": keepalive, "guardrails": {"surveyor_can_promote_canonical_status": False, "evidence_mentions_are_semantic_proof": False, "anti_idle_is_semantic_evidence": False, "keepalive_requires_external_canonical_authority": True}}
    output = args.output_dir.resolve()
    output.mkdir(parents=True, exist_ok=True)
    _json_write(output / "coverage.json", {"schema": "otclient.tibia-re-surveyor.coverage.v1", "generated_at": generated_at, "counts": counts, "rows": coverage_rows})
    _json_write(output / "agent_bundle.json", bundle)
    (output / "summary.md").write_text(_summary_markdown(bundle), encoding="utf-8")
    return bundle
